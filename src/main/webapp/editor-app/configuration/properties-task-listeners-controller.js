...
var KisBpmTaskListenersPopupCtrl = [ '$scope', '$modal', '$q', '$translate', function($scope, $modal, $q, $translate) {
    ...
    // Click handler from select list for add button
    $scope.addNewListenerBySelect = function () {
        var opts = {
            template:  'editor-app/configuration/properties/assignment-customer-listener-popup.html?version=' + Date.now(),
            scope: $scope
        };
    ...
    $scope.setValue = function (choseArr) {
        for(var i=0; i<choseArr.length; i++) {
            ...
            if(choseArr[i] != '') {
                var delegateExpression = choseArr[i].split('_')[1];
