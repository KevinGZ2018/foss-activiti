...
var KisBpmAssignmentCustomerOrgPopupCtrl = [ '$scope', '$http', function($scope, $http) {
    ...
    $scope.initializeController = function () {
        ...
        $http({method: 'GET', url: 'org/list?currentPage=' + $scope.currentPageNum + '&pageSize=' + $scope.pageSize})

            .success(function (data) {

                $scope.content = data;
                $scope.totalElements = 57;
    ...
    $scope.rebuildData = function(content) {
        return 'r_' + content.id + '_' + content.alias + '_' + content.name;
    };
